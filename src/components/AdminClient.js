...
const exportBookings = () => {
  const headers = ['Name', 'Email', 'Phone', 'Amount', 'Status', 'Date', 'Message']
      ...
      >
        ...
        >
          {[
            ...
            { label: 'Messages', value: contactList.length, icon: MessageSquare },
          ].map((s, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
...
{activeTab === 'bookings' && (
  <div>
    ...
    >
      <SectionTitle>All Bookings ({filteredBookings.length})</SectionTitle>
      <motion.button
      ...
      >
        {[
          ...
          <motion.button
            ...
            style={{
              background: bookingFilter === f.id ? '#3E2C23' : '#FFFFFF',
              color: bookingFilter === f.id ? '#C0E1D2' : '#3E2C23',
    ...
    <div className="bookings-mobile" style={{ display: 'none' }}>
      {filteredBookings.map((b) => (
        <BookingCard
          ...
          onUpdateStatus={(status) => handleUpdateBooking(b.id, { status })}
        />
      ))}
    </div>
            ...
            {filteredBookings.map((b, i) => (
              <tr
                ...
                style={{
                  ...
                  background: i % 2 === 0 ? '#fff' : 'rgba(62,44,35,0.02)',
                }}
              >
                <td
                  style={{
                ...
                <td style={{ padding: '14px 16px', color: '#7a5c45' }}>
...
{activeTab === 'contacts' && (
  <div>
    <SectionTitle>Contact Messages ({filteredContacts.length})</SectionTitle>
          ...
          >
            ...
            >
              ...
              <motion.button
                ...
                style={iconBtnStyle('rgba(222,128,43,0.15)', '#DE802B')}
                title="View"
              >
                <Eye size={16} />
              </motion.button>
              <DeleteButton onClick={() => handleDeleteContact(c.id)} />
            </div>
          </div>
          <p
            style={{
        ...
        <AnimatePresence>
          {viewBooking && (
            <BookingDetailModal
              ...
              onUpdate={(updates) => handleUpdateBooking(viewBooking.id, updates)}
            />
          )}
        </AnimatePresence>
...
function StatusBadge({ status }) {
  const config = {
    success: { bg: 'rgba(56,161,105,0.15)', color: '#38a169', icon: CheckCircle, label: 'Confirmed' },
    pending: { bg: 'rgba(214,158,46,0.15)', color: '#d69e2e', icon: Clock, label: 'Pending' },
    failed: { bg: 'rgba(229,62,62,0.15)', color: '#e53e3e', icon: XCircle, label: 'Failed' },
    completed: { bg: 'rgba(62,128,222,0.15)', color: '#3b82f6', icon: CheckCircle, label: 'Completed' },
  }
            ...
            >
              {[
                ...
                <motion.button
                  ...
                  style={{
                    background:
                      booking.status === s.id ? s.color : '#fff',
